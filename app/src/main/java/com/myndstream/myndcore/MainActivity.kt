...
class MainActivity : AppCompatActivity() {
        ...
        private fun testAudioPlayer() {
                try {
                        ...
                        val authed = AuthedHttpClient(authClient, httpClient)
                        val tracking =
                                TrackingClient(
                                        authedHttpClient = authed,
                                        baseUrl = Config.baseApiUrl
                                )
                        val sessionManager = ListeningSessionManager()

                        val audioClient: IAudioClient =
                                        ...
                                        audioClient.events.collect { event ->
                                                when (event) {
                                                        is AudioPlayerEvent.PlaylistQueued -> {
                                                                println(
                                                                        "🎵 Playlist queued: ${event.playlist.playlist.name} with ${event.playlist.songs.size} songs"
                                                                )
                                                        }
                                                        is AudioPlayerEvent.StateChanged -> {
                                                                when (event.state) {
                                                                        ...
                                                                        is PlaybackState.Playing ->
                                                                                println(
                                                                                        "▶️ Player state: Playing - ${(event.state as PlaybackState.Playing).song.name} (track ${(event.state as PlaybackState.Playing).index + 1})"
                                                                                )
                                                                        is PlaybackState.Paused ->
                                                                                println(
                                                                                        "⏸️ Player state: Paused - ${(event.state as PlaybackState.Paused).song.name} (track ${(event.state as PlaybackState.Paused).index + 1})"
                                                                                )
                                                                        is PlaybackState.Stopped ->
                                                                                println(
                                                                                        "⏹️ Player state: Stopped"
                                                                                )
                                                                }
                                                        }
                                                        is AudioPlayerEvent.ProgressUpdated -> {
                                                                ...
                                                                println(
                                                                        "⏱️ Progress: Track ${progress.trackIndex + 1} - ${progress.trackCurrentTime.toInt()}s/${progress.trackDuration.toInt()}s (${(progress.trackProgress * 100).toInt()}%)"
                                                                )
                                                        }
                                                        ...
                                                        is AudioPlayerEvent.SongNetworkFailure -> {
                                                                println(
                                                                        "❌ Network failure: ${event.error.message}"
                                                                )
                                                        }
